...
class fileViewModel {
  ...
  */
  ...
  _getStatementStyleTwo(line) {
    ...
    let fromTo = fromParts[1].split(':')[0]
    let from, to
    let splitter = null;
    if (fromTo.indexOf(" To ") > -1) {
      splitter = " To ";
    }
    else if (fromTo.indexOf(" to ") > -1) {
      splitter = " to "
    }
    if (splitter) {
      let parts = fromTo.split(splitter);
      from = parts[0].trim();
      to = parts[1].trim().split(":")[0];
    } else {
      from = fromTo.trim();
      to = "Everyone";
    }
    ...
    let statement =
    {
      ...
      from: from,
      to: to,
      contents: ""  // Contents will be added in the main parsing loop
    };
    ...
    for (var line of lines) {
      ...
      if (timeRegex.test(line)) {
        ...
        // line doesn't contain "From" or "to"
          ...
          // Parse "to Everyone" format
          let toParts = fromParts[1].split(" to ");
          statementInProgress.from = toParts[0].trim();
          statementInProgress.to = toParts[1].split(":")[0].trim();
        }

      } else if (statementInProgress != null) {
        // Add line to contents, preserving only one newline between content lines
        if (statementInProgress.contents && line.trim()) {
          statementInProgress.contents += "\n" + line.trim();
        } else {
          statementInProgress.contents += line.trim();
        }
      }
    }
    if (statementInProgress != null) {
